...
class Point(object):
    ...
    def move_to(self,x,y):
        self.distancetraveled=self.distancetraveled+self.get_distance_from(Point(x,y))
        self.x=x
        self.y=y
        self.moves=self.moves+1
        return self
    def move_by(self,dx,dy):
        self.distancetraveled=self.distancetraveled+self.get_distance_from(Point(self.x+dx,self.y+dy))
        self.x=self.x+dx
        self.y=self.y+dy
        self.moves = self.moves + 1
    ...
    def get_distance_from(self,p1):
        value = math.sqrt((self.x-p1.x)**2+(self.y-p1.y)**2)
        return value
    def get_distance_from_start(self):

        distance=self.get_distance_from(Point(self.a,self.b))
        return distance
    def get_distance_traveled(self):
        return self.distancetraveled
    def closer_to(self,p1,p2):
        value1=self.get_distance_from(p1)
        value2=self.get_distance_from(p2)
        if value1>value2:
            return p2
        else:
            return p1
    def halfway_to(self,p1):
        p2=Point((self.x+p1.x)/2,(self.y+p1.y)/2)
        return p2
def run_test_init():
    """
